...
{
	class ControlsScreen : public Ebony::Screen
	{
	public:
		...
		void OnScreenDefocus(std::uint64_t lastScreenEnum) override;
		void OnScreenFocus(std::uint64_t lastScreenEnum) override;

		Ebony::RenderTarget2D mainRenderTarget;
		std::uint64_t screen = EcologyRTS::ScreenEnum::CONTROLS;
		std::uint64_t nextScreen = EcologyRTS::ScreenEnum::CONTROLS;
