...
namespace EcologyRTS
{
	...
	void EcologyTiledProcessor::ParseMap(const char* filepath)
	{
		//tson::Tileson t;
		...
		//	tson::Tileset* tileset = map->getTileset("EcologyRTSTileset");
		//		
		...
		//		auto sprite = std::make_unique<components::Sprite>(Ebony::ResourceManager::GetShader("spritesheet"), Ebony::ResourceManager::GetTexture("EcologyRTS_spritesheet"), Ebony::Colors::White, Ebony::RenderLayer::BACKGROUND, false, 0.0f, index, true);
	...
	void EcologyTiledProcessor::CreateTranslationFunction()
	{
		/*translationFunction = [=](glm::vec2 position, int entityId) 
			{
				...
				{
				case 577:
					Ebony::SystemManager::AddEntity(EcologyRTS::Bat::Create(position, glm::vec2(1.0f), player));
					break;
				case 578:
					Ebony::SystemManager::AddEntity(EcologyRTS::SuicideBird::Create(position, glm::vec2(1.0f), player));
					break;
				default:
