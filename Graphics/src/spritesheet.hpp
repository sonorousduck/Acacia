...
{
public:
	SpriteSheet(Texture2D spritesheet, std::uint8_t numDivisions, std::vector<float> timings) : spritesheet(spritesheet), numDivisions(numDivisions), timings(timings)
	{
		divisions = std::vector<std::pair<glm::vec2, glm::vec2>>();
	}

	SpriteSheet(Texture2D spritesheet, std::vector<std::pair<glm::vec2, glm::vec2>> divisions, std::vector<float> timings) : spritesheet(spritesheet), divisions(divisions), timings(timings)
	{
		numDivisions = divisions.size();
	}
