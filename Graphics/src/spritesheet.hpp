...
{
public:
	SpriteSheet(std::shared_ptr<Texture2D> spritesheet, std::uint8_t numDivisions, std::vector<std::chrono::microseconds> timings) : spritesheet(spritesheet), numDivisions(numDivisions), timings(timings)
	{
		divisions = std::vector<std::pair<glm::vec2, glm::vec2>>();
	}

	SpriteSheet(std::shared_ptr<Texture2D> spritesheet, std::vector<std::pair<glm::vec2, glm::vec2>> divisions, std::vector<std::chrono::microseconds> timings) : spritesheet(spritesheet), divisions(divisions), timings(timings)
	{
