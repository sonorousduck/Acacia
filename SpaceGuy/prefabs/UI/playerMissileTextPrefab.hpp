...
namespace SpaceGuy
{
	...
	public:
		static entities::EntityPtr Create(glm::vec2 startPosition, entities::EntityPtr player)
		{
			...
			std::unique_ptr<components::CppScript> script = std::make_unique<scripts::PlayerMissileScript>(player);

			entity->addComponent(std::make_unique<components::Transform>(startPosition + glm::vec2(8.0f, 0.0f), 0.0f, glm::vec2(1.0f, 1.0f)));
