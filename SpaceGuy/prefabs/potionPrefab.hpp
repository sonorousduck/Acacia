...
namespace SpaceGuy
{
	...
	public:
		...
		static entities::EntityPtr Create(glm::vec2 startTransform)
		{
			...
			auto sprite = std::make_unique<components::Sprite>(Ebony::ResourceManager::GetShader("default"), Ebony::ResourceManager::GetTexture("potion"), Ebony::Colors::White, Ebony::RenderLayer::FOREGROUND);
			...
			aabbcollider.onCollisionStart = [=](entities::EntityPtr other, std::chrono::microseconds elapsedTime)
				{
					...
					playerInformation->health += 6.0f;
					playerInformation->health = std::min(playerInformation->health, playerInformation->maxHealth);
