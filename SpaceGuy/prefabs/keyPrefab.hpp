...
namespace SpaceGuy
{
	...
	public:
		...
		static entities::EntityPtr Create(glm::vec2 startTransform)
		{
			...
			auto sprite = std::make_unique<components::Sprite>(Ebony::ResourceManager::GetShader("default"), Ebony::ResourceManager::GetTexture("key"), Ebony::Colors::White, Ebony::RenderLayer::FOREGROUND);
			...
			aabbcollider.onCollisionStart = [=](entities::EntityPtr other, std::chrono::microseconds elapsedTime)
				{
					...
					if (!playerInformation->hasKey)
					{
						playerInformation->hasKey = true;
