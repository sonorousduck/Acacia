...
namespace SpaceGuy
{
	...
	public:
		...
		static entities::EntityPtr Create(glm::vec2 startTransform)
		{
			...
			auto sprite = std::make_unique<components::Sprite>(Ebony::ResourceManager::GetShader("default"), Ebony::ResourceManager::GetTexture("shot_spread_powerup"), Ebony::Colors::White, Ebony::RenderLayer::FOREGROUND);
			...
			aabbcollider.onCollisionStart = [=](entities::EntityPtr other, std::chrono::microseconds elapsedTime)
				{
					...
					playerInformation->hasShotgun = true;
					playerInformation->hasRapidFire = false;

					other->getComponent<components::TimedComponent>()->maxTime = 0.60f;
