...
namespace SpaceGuy
{
	...
	public:
		...
		static entities::EntityPtr Create(glm::vec2 startTransform)
		{
			...
			auto sprite = std::make_unique<components::Sprite>(Ebony::ResourceManager::GetShader("default"), Ebony::ResourceManager::GetTexture("fast_shooting_powerup"), Ebony::Colors::White, Ebony::RenderLayer::FOREGROUND);
			...
			aabbcollider.onCollisionStart = [=](entities::EntityPtr other, std::chrono::microseconds elapsedTime)
				{
					auto playerInformation = other->getComponent<components::PlayerShootingInformation>();

					playerInformation->hasShotgun = false;
					playerInformation->hasRapidFire = true;

					other->getComponent<components::TimedComponent>()->maxTime = 0.12f;
