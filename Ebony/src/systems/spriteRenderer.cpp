	...
	void SpriteRenderer::Update()
	{
		for (auto&& [id, entity] : m_Entities)
		{
			...
			if (debug && entity->hasComponent<components::Collider>())
			{
				...
				Ebony::Graphics2d::Draw(Ebony::ResourceManager::GetShader("default"), Ebony::ResourceManager::GetTexture("collider"), transform->position + collider->aabbCollider.getCenter() - (collider->aabbCollider.getSize() / 2.0f), collider->aabbCollider.getSize(), transform->rotation, transform->rotationAxis, Ebony::Colors::Red, sprite->depth + 0.01f, sprite->isUI);
			}
